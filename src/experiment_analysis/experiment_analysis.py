...
def get_from_key(algorithm_name, data_set_name, key: str):
    ...
    The key is in ['time_train', 'time_test','error_train', 'error_test', 'degree', 'total_sparsity',
    ...
    elif key in ['error_train', 'error_test', 'total_sparsity', 'degree', 'number_of_polynomials_and_terms', 'G + O']:
        mean = "{:.2f}".format(mean)
        std = "{:.2f}".format(std)
    return mean, std
...
def table_results(data_sets, algorithms, key, ordering=False):
    ...
    The key is in ["error train", "error test", "time train", "time test", "time hyper", "degree", "total sparsity",
    "G + O"].
    """
