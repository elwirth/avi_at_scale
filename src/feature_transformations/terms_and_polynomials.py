...
class SetsAVI:
    ...
    def apply_G_transformation(self, X_test: cp.ndarray):
        """Applies the transformation corresponding to G to X_test."""
        test_sets_avi = SetsAVI(X_test, m=self.m)
        i = 1
    ...
    def construct_border(self):
        ...
        if len(self.O_terms) == 1:
            ...
            border_terms_raw, border_evaluations_raw, non_purging_indices = construct_border(fd(self.O_terms[-1]),
                                                                                             fd(self.O_evaluations[-1]),
                                                                                             fd(self.X),
                                                                                             fd(self.O_terms[1]),
                                                                                             fd(self.O_evaluations[1]),
                                                                                             self.leading_terms)
