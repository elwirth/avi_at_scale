...
class ExperimentSetups:
    ...
    def hyperparameter_combinations_oavi(self):
        ...
        columns = ['oracle_type', 'term_ordering_strategy', 'inverse_hessian_boost', 'psi', 'C', 'time_t', 'time_v',
                   'accuracy_t', 'accuracy_v', 'avg_sparsity', 'total_sparsity', 'number_of_polynomials',
            ...
            for C in Cs:
                ...
                for idx in range(0, len(X_train_chunks)):
                    ...
                    oracle_avi = OracleAVI(psi=psi, oracle_type=oracle_type,
                                           term_ordering_strategy=term_ordering_strategy,
                ...
                row = pd.DataFrame([[oracle_type, term_ordering_strategy, inverse_hessian_boost, psi, C, time_t_avg,
                                     time_v_avg, acc_t_avg, acc_v_avg, sparsity_avg, tot_sparsity_avg, n_poly_avg,
                                     n_zeros_avg, n_entries_avg, n_terms_avg, degree_avg]],
                                   columns=columns)
                df = df.append(row, ignore_index=True)
    ...
    def final_training_oavi(self, data_frame: pd.DataFrame, count_svm: bool = True):
        """Performs the final retraining for OAVI and linear kernel SVM, with the optimal hyperparameter choices."""
        columns = ['oracle_type', 'term_ordering_strategy', 'inverse_hessian_boost', 'psi', 'C', 'time_train',
                   'time_test', 'accuracy_train', 'accuracy_test', 'avg_sparsity', 'total_sparsity',
                   'number_of_polynomials', 'number_of_zeros', 'number_of_entries', 'number_of_terms', 'degree']
        oracle_type = self.hyperparameters["oracle_type"]
        term_ordering_strategy = self.hyperparameters["term_ordering_strategy"]
        inverse_hessian_boost = self.hyperparameters["inverse_hessian_boost"]
        ...
        df_test = pd.DataFrame(
            [[oracle_type, term_ordering_strategy, inverse_hessian_boost, psi, C, time_train, time_test, acc_train,
              acc_test, avg_sparsity, tot_sparsity, n_poly, n_zeros, n_entries, n_terms, degree]], columns=columns)
        return df_test
