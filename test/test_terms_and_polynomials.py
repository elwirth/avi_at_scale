...
from src.feature_transformations.terms_and_polynomials import SetsAVI, SetsVCA


class TestSetsAVI(unittest.TestCase):

    def test_evaluate_transformation(self):
        """Tests whether SetsAVI.evaluate_transformation() beahves as intended."""
        X_train = cp.array([[1, 2],
                            [3, 4],
                            [5, 6]])

        sets_avi = SetsAVI(X_train)
