...
class TestOracleAVI(unittest.TestCase):
    ...
    def test_evaluate(self):
        ...
        for oracle in ["CG", "AGD", "ABM"]:
            for inverse_hessian_boost in ["false",  "full"]:
                if inverse_hessian_boost:
                    lmbda = 0
                else:
                    lmbda = 0.1
                if (oracle == "ABM") and (inverse_hessian_boost is not "false"):
                    pass
                else:
                    o_avi = OracleAVI(psi=self.psi, eps=self.psi, lmbda=lmbda,
                    ...
                    o_avi = OracleAVI(psi=0.0045, eps=0.0045, lmbda=lmbda, max_degree=4,
                                      inverse_hessian_boost=inverse_hessian_boost, oracle_type=oracle)
